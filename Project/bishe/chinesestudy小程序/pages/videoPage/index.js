...
Page({
    ...
    onLoad: function (options) {
        ...
        this.startTime = options.startTime ? parseInt(options.startTime, 10) : 0;

        // 获取用户信息后再初始化视频详情和记录开始时间
        this.getUserByToken().then(() => {
            this.getVideoDetail(videoId).then(() => {
                // Create video context and play video after data is loaded
    ...
    onReady: function () {
        ...
        // 自动跳转到指定时间
        this.videoContext.seek(this.startTime);
        this.videoContext.play(); // 确保视频开始播放
    },
    ...
    async getVideoDetail(videoId) {
        ...
        const likeCountResult = await requestUtil({ url: "/video/interaction/getLikeCount", data: { videoId } });

        this.setData({
            baseUrl,
            video: videoResult.message,
            comments: formatComments,
            likeCount: likeCountResult.likeCount // 新增：从接口获取点赞个数
        });
    ...
    async toggleLike() {
        ...
        if (this.data.isLiked) {
            ...
            this.setData({
                isLiked: false,
                likeCount: this.data.likeCount - 1 // 点赞数减一
            });
        } else {
            // 点赞
            await requestUtil({
            ...
            this.setData({
                isLiked: true,
                likeCount: this.data.likeCount + 1 // 点赞数加一
            });
        }
    },
